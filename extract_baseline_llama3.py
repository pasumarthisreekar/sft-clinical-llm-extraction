...
def extract_clinical_data(text: str) -> str:
    """Sends clinical text to local Llama-3 with safety limits to prevent freezing."""
...
def run_extraction():
    print("Loading Evaluation Data (JSONL)...")
    df = pd.read_json("data/processed/eval_notes.jsonl", orient="records", lines=True)
    total_notes = len(df)
    
    # Load existing results to support seamless resuming if it crashed earlier
    output_path = "base_model_eval_results.json"
    results = []
    processed_ids = set()
    ...
    print(f"\nFinished! Saved all baseline extractions to {output_path}")
